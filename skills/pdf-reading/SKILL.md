...
tags: pdf, extraction, ocr, inspection
compatibility: Requires the snapshot-bundled PDF extraction and OCR runtimes.
---
...
This guide covers essential PDF reading operations using Python libraries and command-line tools. For advanced features (pypdfium2 rendering, pdfplumber table settings, OCR fallback, encrypted/corrupted PDF handling), see REFERENCE.md in the pdf skill.
...
→ Rasterize pages at 150 DPI and Read them visually. For bulk text
  ...
  see REFERENCE.md in the pdf skill for a complete example).
...
For tables that need DataFrame output:
```python
import pandas as pd
import pdfplumber
...
other PDF manipulation tasks, use the `pdf` skill.
