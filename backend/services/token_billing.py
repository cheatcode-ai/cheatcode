...
from utils.constants import get_credits_from_tokens, get_plan_by_id
...
async def consume_tokens(
    ...
    try:
        ...
        if plan_id == "byok":
            ...
            try:
                ...
                if user_api_key and key_source == "user_byok":
                    ...
                    estimated_cost = await OpenRouterPricing.estimate_cost_for_model(model, tokens_used, user_api_key)
                else:
                ...
                logger.error(f"Error calculating real OpenRouter cost for BYOK user {account_id}: {e!s}")
        ...
        logger.error(f"Error consuming tokens for user {account_id}: {e!s}")
        raise
...
async def _consume_tokens_fallback(
    ...
    try:
        ...
        try:
            ...
            update_result = await client.rpc(
            ...
            if rpc_result and rpc_result.get("success") is True:
                ...
                if error_type == "insufficient_tokens":
                    raise InsufficientTokensError(
                        error_message, tokens_remaining, get_credits_from_tokens(tokens_remaining)
                    )
            ...
            logger.warning(f"RPC consume_tokens_atomic failed, using fallback: {rpc_error!s}")
            update_result = (
                await client.table("users")
                .update(
                    {
            ...
            if not update_result.data or len(update_result.data) == 0:
                ...
                fresh_result = (
                    await client.table("users").select("token_quota_remaining").eq("id", account_id).execute()
                )
        ...
        estimated_cost = calculate_token_cost(tokens_used // 2, tokens_used // 2, model)
        ...
        logger.info(
            f"Consumed {tokens_used} tokens for user {account_id}. Remaining: {new_remaining} tokens ({remaining_credits} credits)"
        )
        ...
        try:
            ...
            logger.debug(f"Failed to invalidate billing cache: {cache_error!s}")
        ...
        logger.error(f"Error in fallback token consumption for user {account_id}: {e!s}")
        raise
...
async def get_user_token_status(client: Client, account_id: str) -> dict:
    ...
    try:
        ...
        billing_result = await client.table("users").select("*").eq("id", account_id).execute()
        ...
        logger.error(f"Error getting token status for user {account_id}: {e!s}")
        raise
...
async def _create_billing_record(client: Client, account_id: str) -> None:
    ...
    try:
        ...
        logger.warning(
            f"Creating fallback user record for user {account_id} - this should have been created during signup"
        )
            ...
            .insert(
                {
                    ...
                    "quota_resets_at": (datetime.now(tz=UTC) + timedelta(days=30)).isoformat(),
                    "billing_updated_at": datetime.now(tz=UTC).isoformat(),
                }
            )
            .execute()
        ...
        logger.error(f"Error creating billing record for user {account_id}: {e!s}")
        raise
...
async def get_token_usage_history(client: Client, account_id: str, days: int = 30) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"JOIN with threads table failed, falling back to basic query: {join_error!s}")
            result = (
                await client.table("token_usage_log")
                .select("*")
                .eq("user_id", account_id)
        ...
        logger.error(f"Error getting token usage history for user {account_id}: {e!s}")
        raise
