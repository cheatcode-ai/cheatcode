...
# Process-local cache for OpenRouter models data.
# Avoids Redis GET on every pricing lookup within the same process.
_local_models_cache: dict | None = None
_local_models_cache_expiry: float = 0.0
_LOCAL_MODELS_CACHE_TTL = 1800  # 30 minutes local TTL
...
class OpenRouterPricing:
    ...
    async def get_model_pricing(model_name: str, api_key: str) -> dict[str, float]:
        ...
        try:
            ...
            clean_model_name = (
                model_name.replace("openrouter/", "") if model_name.startswith("openrouter/") else model_name
            )
            ...
            logger.error(f"Error getting OpenRouter pricing for {model_name}: {e!s}")
            return OpenRouterPricing._get_default_pricing()

    @staticmethod
    async def _get_cached_models() -> dict | None:
        """Get cached models data (local → Redis).

        Returns:
            Cached models data or None if cache miss/error

        """
        global _local_models_cache, _local_models_cache_expiry

        # Tier 1: process-local cache (0 Redis commands)
        if _local_models_cache and time.monotonic() < _local_models_cache_expiry:
        ...
        try:
            ...
            if cached_data:
                ...
                _local_models_cache = parsed
                _local_models_cache_expiry = time.monotonic() + _LOCAL_MODELS_CACHE_TTL
                return parsed
            return None
        except Exception as e:
            logger.warning(f"Redis cache read error (graceful fallback to API): {e!s}")
            return None

    @staticmethod
    async def _cache_models_data(models_data: dict) -> None:
        """Cache models data in both local and Redis caches.
        ...
        _local_models_cache = models_data
        _local_models_cache_expiry = time.monotonic() + _LOCAL_MODELS_CACHE_TTL

        # Populate Redis cache
        try:
            await redis.set(OPENROUTER_CACHE_KEY, json.dumps(models_data), ex=CACHE_TTL_SECONDS)
            logger.info(f"Cached OpenRouter models data for {CACHE_TTL_HOURS} hours")
        except Exception as e:
            logger.warning(f"Redis cache write error (non-critical): {e!s}")

    @staticmethod
    async def _fetch_openrouter_models(api_key: str) -> dict | None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session, session.get(url, headers=headers, timeout=10) as response:
            ...
            logger.error(f"Error fetching OpenRouter models: {e!s}")
            return None

    @staticmethod
    def _extract_model_pricing(models_data: dict, clean_model_name: str) -> dict[str, float] | None:
        ...
        try:
            ...
            logger.error(f"Error extracting model pricing: {e!s}")
            return None

    @staticmethod
    def _get_default_pricing() -> dict[str, float]:
        """Return default pricing when OpenRouter API is unavailable."""
    ...
    @staticmethod
    def calculate_real_cost(prompt_tokens: int, completion_tokens: int, pricing_info: dict[str, float]) -> float:
        ...
        try:
            ...
            logger.error(f"Error calculating real cost: {e!s}")
            return 0.0
    ...
    async def estimate_cost_for_model(
        ...
        try:
            ...
            return OpenRouterPricing.calculate_real_cost(prompt_tokens, completion_tokens, pricing_info)

        except Exception as e:
            logger.error(f"Error estimating cost for {model_name}: {e!s}")
            return 0.0
    ...
    async def clear_cache() -> bool:
        """Clear both local and Redis OpenRouter pricing caches.
        ...
        _local_models_cache_expiry = 0.0

        try:
            result = await redis.delete(OPENROUTER_CACHE_KEY)
            logger.info(f"OpenRouter pricing cache cleared (deleted: {result})")
            return bool(result)
        except Exception as e:
    ...
    async def get_cache_info() -> dict[str, any]:
        ...
        try:
            ...
            return {"cached": False, "error": str(e), "cache_key": OPENROUTER_CACHE_KEY}
