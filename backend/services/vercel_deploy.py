...
class VercelDeploymentService:
    ...
    async def ensure_project(self, project_name: str, framework: str = "nextjs") -> dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            resp = await client.get(
                f"{VERCEL_API_BASE}/v9/projects/{project_name}", headers=self._headers(), params=self._team_params()
            )
    ...
    async def deploy_files(
        self, project_name: str, files: list[dict[str, str]], target: str = "production", framework: str = "nextjs"
    ) -> dict[str, Any]:
        ...
        project_settings = {"framework": framework, "installCommand": "npm install"}
        if framework == "nextjs":
...
async def collect_files_for_vercel(sandbox, workdir: str = "/workspace/cheatcode-app") -> list[dict[str, str]]:
    ...
    try:
        ...
        result = await sandbox.process.exec(f"find {workdir} -type f {exclude_args}", timeout=60)
    ...
    async def download_file(file_path: str) -> dict[str, str] | None:
        ...
        try:
            ...
            if is_binary:
                return {"file": relative_path, "data": base64.b64encode(content).decode("utf-8"), "encoding": "base64"}
            # Try UTF-8 decode, fall back to base64 for binary content
            try:
                text_content = content.decode("utf-8")
                return {"file": relative_path, "data": text_content, "encoding": "utf-8"}
    ...
    for batch_idx in range(total_batches):
        ...
        results = await asyncio.gather(*[download_file(p) for p in batch], return_exceptions=True)
