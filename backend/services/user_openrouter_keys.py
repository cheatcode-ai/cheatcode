...
class OpenRouterKeyManager:
    ...
    @staticmethod
    async def store_api_key(account_id: str, api_key: str, display_name: str = "OpenRouter API Key") -> str:
        ...
        try:
            ...
            logger.error(f"Error storing OpenRouter API key for user {account_id}: {e!s}")
            raise

    @staticmethod
    async def get_api_key(account_id: str) -> str | None:
        ...
        try:
            ...
            logger.error(f"Error retrieving OpenRouter API key for user {account_id}: {e!s}")
            return None

    @staticmethod
    async def get_key_info(account_id: str) -> OpenRouterKeyInfo | None:
        ...
        try:
            ...
            logger.error(f"Error getting OpenRouter key info for user {account_id}: {e!s}")
            return None
    ...
    async def delete_api_key(account_id: str) -> bool:
        ...
        try:
            db = DBConnection()
            async with db.get_async_client() as client:
                await client.table("user_openrouter_keys").delete().eq("user_id", account_id).execute()

                logger.info(f"Deleted OpenRouter API key for user {account_id}")
                return True

        except Exception as e:
            logger.error(f"Error deleting OpenRouter API key for user {account_id}: {e!s}")
            return False
    ...
    async def update_last_used(account_id: str) -> None:
        ...
        try:
            ...
            async with db.get_async_client() as client:
                await (
                    client.table("user_openrouter_keys")
            ...
            logger.error(f"Error updating last_used_at for user {account_id}: {e!s}")
    ...
    async def set_key_active_status(account_id: str, is_active: bool) -> bool:
        ...
        try:
            ...
            logger.error(f"Error setting key active status for user {account_id}: {e!s}")
            return False
    ...
    async def test_api_key_connection(api_key: str) -> dict[str, Any]:
        ...
        try:
            ...
            async with (
                ...
                session.get(key_status_url, headers=headers, timeout=10) as response,
            ):
                ...
                if response.status == 401:
                    return {"success": False, "error": "Invalid OpenRouter API key or expired session"}
            ...
            return {"success": False, "error": f"Connection test failed: {e!s}"}
