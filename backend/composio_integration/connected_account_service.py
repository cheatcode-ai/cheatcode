...
class ConnectedAccountService:
    ...
    async def _get_or_create_auth_config_for_toolkit(self, toolkit_slug: str) -> str | None:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    ...
                    params={"toolkit_slug": toolkit_lower, "is_composio_managed": "true", "limit": 1},
                )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{COMPOSIO_API_V3}/auth_configs", headers=_get_headers(), json={"toolkit": {"slug": toolkit_lower}}
                )
            ...
            logger.error(
                f"HTTP error creating auth config for {toolkit_slug}: {e.response.status_code} - {e.response.text}"
            )
            return None
        except Exception as e:
    ...
    async def initiate_connection(
        ...
        auth_mode: str | None = None,  # noqa: ARG002
        auth_config: dict[str, Any] | None = None,  # noqa: ARG002
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{COMPOSIO_API_V3}/connected_accounts", headers=_get_headers(), json=payload
                )
                response.raise_for_status()
                data = response.json()
            ...
            connection_id = connection.get("id") or data.get("connectionId") or data.get("connection_id") or ""

            # v3 returns redirect info in different places
            redirect = (
                connection.get("redirectUrl")
                or connection.get("redirect_url")
                or data.get("redirectUrl")
                or data.get("redirect_url")
                or data.get("redirect_uri")
                or connection.get("data", {}).get("redirectUrl")
                or connection.get("data", {}).get("authUri")
                or ""
            )
    ...
    async def get_connection(self, connection_id: str) -> ConnectedAccount | None:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{COMPOSIO_API_V3}/connected_accounts/{connection_id}", headers=_get_headers()
                )
    ...
    async def get_connection_status(self, connection_id: str) -> ConnectionStatus:
        ...
        try:
            ...
            return ConnectionStatus(id=connection_id, status="ERROR", error=str(e))

    async def list_connections(
        self, user_id: str, app_name: str | None = None, status: str | None = None
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{COMPOSIO_API_V3}/connected_accounts", headers=_get_headers(), params=params
                )
                response.raise_for_status()
                data = response.json()
    ...
    async def wait_for_connection(self, connection_id: str, timeout: int = 60) -> ConnectedAccount:
    ...
    async def refresh_connection(self, connection_id: str) -> ConnectedAccount | None:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{COMPOSIO_API_V3}/connected_accounts/{connection_id}/refresh", headers=_get_headers()
                )
    ...
    async def get_user_connections_by_app(self, user_id: str) -> dict[str, list[ConnectedAccount]]:
    ...
    def _convert_dict_to_connected_account(self, data: dict[str, Any]) -> ConnectedAccount | None:
        """Convert REST API v3 response dict to ConnectedAccount model."""
        try:
            account_id = str(data.get("id", ""))

            # v3 uses user_id, v1 used entityId
            entity_id = str(data.get("user_id") or data.get("entityId") or data.get("entity_id") or "")

            # v3 uses toolkit object with slug, v1 used appName
            toolkit = data.get("toolkit", {})
            app_name = (toolkit.get("slug") if isinstance(toolkit, dict) else None) or (
                data.get("appName") or data.get("app_name") or data.get("app") or ""
            )
            ...
            connection_params = (
                data.get("connectionParams")
                or data.get("connection_params")
                or data.get("data")
                or data.get("val")
                or {}
            )
