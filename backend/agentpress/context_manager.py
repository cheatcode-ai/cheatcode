...
class ContextManager:
    ...
    def compress_message(
        self, msg_content: str | dict, message_id: str | None = None, max_length: int = 3000
    ) -> str | dict:
        ...
        if isinstance(msg_content, dict):
            if len(json.dumps(msg_content)) > max_length:
                # Special handling for read_file tool result to preserve JSON structure
                tool_execution = msg_content.get("tool_execution", {})
                if tool_execution.get("function_name") == "read_file":
                    output = tool_execution.get("result", {}).get("output", {})
                    if (
                        isinstance(output, dict)
                        and isinstance(output.get("content"), str)
                        and len(output["content"]) > max_length // 4
                    ):
                        # Truncate file content within the JSON
                        output["content"] = output["content"][: max_length // 4] + "\n... (truncated)"

                # Special handling for edit_file tool result to preserve JSON structure
                elif tool_execution.get("function_name") == "edit_file":
                    output = tool_execution.get("result", {}).get("output", {})
                    if isinstance(output, dict):
                        # Truncate both original_content and updated_content fields
                        for key in ["original_content", "updated_content"]:
                            if isinstance(output.get(key), str) and len(output[key]) > max_length // 6:
                                output[key] = output[key][: max_length // 6] + "\n... (truncated)"
    ...
    def compress_tool_result_messages(
        self, messages: list[dict[str, Any]], llm_model: str, max_tokens: int | None, token_threshold: int = 1000
    ) -> list[dict[str, Any]]:
    ...
    def compress_messages(
        ...
        logger.info(
            f"compress_messages: {uncompressed_total_token_count} -> {compressed_token_count}"
        )  # Log the token compression for debugging later

        if max_iterations <= 0:
            logger.warning("compress_messages: Max iterations reached, omitting messages")
            return self.compress_messages_by_omitting_messages(messages, llm_model, max_tokens)
    ...
    def compress_messages_by_omitting_messages(
        ...
        while current_token_count > max_allowed_tokens and safety_limit > 0:
            ...
            if len(conversation_messages) <= min_messages_to_keep:
                logger.warning(
                    f"Cannot compress further: only {len(conversation_messages)} messages remain (min: {min_messages_to_keep})"
                )
                break
        ...
        logger.info(
            f"compress_messages_by_omitting_messages: {initial_token_count} -> {final_token_count} tokens ({len(messages)} -> {len(final_messages)} messages)"
        )
