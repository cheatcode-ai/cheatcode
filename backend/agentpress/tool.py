...
class XMLTagSchema:
    ...
    def add_mapping(self, param_name: str, node_type: str = "element", path: str = ".", required: bool = True) -> None:
        ...
        """
        self.mappings.append(XMLNodeMapping(param_name=param_name, node_type=node_type, path=path, required=required))
        logger.debug(
...
class ToolSchema:
    ...
    schema: dict[str, Any]
    xml_schema: XMLTagSchema | None = None
...
def openapi_schema(schema: dict[str, Any]):
    """Decorate function with OpenAPI schema."""
...
def xml_schema(tag_name: str, mappings: list[dict[str, Any]] | None = None, example: str | None = None):
    """Decorate function with XML schema and improved node mapping.
    ...
    Example:
        @xml_schema(
            ...
            example='''
            <edit-file target_file="path/to/file">
                <instructions>Add error handling to login function</instructions>
                <code_edit>
    // ... existing code ...
...
def custom_schema(schema: dict[str, Any]):
    """Decorate function with custom schema."""

    def decorator(func):
        logger.debug(f"Applying custom schema to function {func.__name__}")
        return _add_schema(func, ToolSchema(schema_type=SchemaType.CUSTOM, schema=schema))
