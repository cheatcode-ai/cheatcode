...
class ResponseProcessor:
            ...
            if pending_tool_executions:
                ...
                for execution in pending_tool_executions:
                    ...
                    if tool_idx in yielded_tool_indices:
                        logger.debug(f"Status for tool index {str(tool_idx)} already yielded.")
                        ...
                        try:
                            if execution["task"].done():
                                ...
                                self.trace.event(
                                    ...
                                    level="WARNING",
                                    status_message=(
                                        f"Task for tool index {str(tool_idx)} not done after wait."
                                    ),
                                )
                        except Exception as e:
    ...
    async def _execute_tools_sequentially(self, tool_calls: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], ToolResult]]:
        ...
        try:
            ...
            error_results = [(tool, ToolResult(success=False, output=f"Execution error: {error_str}")) 
    ...
    def _create_structured_tool_result(self, tool_call: Dict[str, Any], result: ToolResult, parsing_details: Optional[Dict[str, Any]] = None, for_llm: bool = False):
        ...
        if for_llm and function_name in ['read_file'] and isinstance(output, dict):
