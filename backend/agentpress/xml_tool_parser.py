...
class XMLToolParser:
    """Parse XML tool calls using the Cursor-style format.
    ...
    def parse_content(self, content: str) -> list[XMLToolCall]:
        ...
        for fc_content in function_calls_matches:
            ...
            for function_name, invoke_content in invoke_matches:
                try:
                    tool_call = self._parse_invoke_block(function_name, invoke_content, fc_content)
                    if tool_call:
                        tool_calls.append(tool_call)
                except Exception:
                    logger.exception(f"Error parsing invoke block for {function_name}")
    ...
    def _parse_invoke_block(self, function_name: str, invoke_content: str, full_block: str) -> XMLToolCall | None:
        """Parse a single invoke block into an XMLToolCall."""
        parameters = {}
        parsing_details = {"format": "v2", "function_name": function_name, "raw_parameters": {}}
        ...
        invoke_pattern = re.compile(
            rf'<invoke\s+name=["\']{re.escape(function_name)}["\']>.*?</invoke>', re.DOTALL | re.IGNORECASE
        )
        ...
        return XMLToolCall(
            function_name=function_name, parameters=parameters, raw_xml=raw_xml, parsing_details=parsing_details
        )
    ...
    def format_tool_call(self, function_name: str, parameters: dict[str, Any]) -> str:
        """Format a tool call as Cursor-style XML.
    ...
    def validate_tool_call(
        self, tool_call: XMLToolCall, expected_params: dict[str, type] | None = None
    ) -> tuple[bool, str | None]:
        """Validate a tool call against expected parameter definitions.
