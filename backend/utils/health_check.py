...
class HealthChecker:
    ...
    def __init__(self, instance_id: str, interval: int = 600):
        self.instance_id = instance_id
        self.interval = interval
        self.last_check = {}

    async def check_redis_health(self) -> HealthStatus:
        """Check Redis connectivity via PING (1 command instead of 4)."""
    ...
    async def check_concurrency_health(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus(
                component="concurrency",
                status="unknown",
                message=f"Concurrency health check failed: {e!s}",
                timestamp=time.time(),
                details={"error": str(e)},
            )
    ...
    async def run_comprehensive_health_check(self) -> dict[str, Any]:
        ...
        for i, check_result in enumerate(checks):
            ...
            if isinstance(check_result, Exception):
                results[component] = HealthStatus(
                    component=component,
                    status="unknown",
                    message=f"Health check failed: {check_result!s}",
                    timestamp=time.time(),
        ...
        try:
            ...
            await redis.set(health_key, json.dumps(health_data), ex=self.interval * 2)
        except Exception as e:
        ...
        if overall_status == "critical":
            ...
            logger.info("Health check HEALTHY: All systems operational")
...
def get_health_checker(instance_id: str, interval: int = 600) -> HealthChecker:
    """Get or create the global health checker."""
    global _health_checker
    if _health_checker is None:
        _health_checker = HealthChecker(instance_id, interval=interval)
    return _health_checker


async def start_health_monitoring(instance_id: str, interval: int = 600):
    """Start periodic health monitoring."""
    health_checker = get_health_checker(instance_id, interval=interval)
