...
from sandbox.sandbox import delete_sandbox, get_or_start_sandbox, get_sandbox
from services.supabase import DBConnection
from utils.auth_utils import get_account_id_for_clerk_user, get_optional_user_id
...
def normalize_path(path: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Error processing Unicode escapes in path '{path}': {unicode_err!s}")

        logger.debug(f"Normalized path from '{path}' to '{decoded_path}'")
        return decoded_path
    except Exception as e:
        logger.error(f"Error normalizing path '{path}': {e!s}")
        return path  # Return original path if decoding fails


async def verify_sandbox_access(client, sandbox_id: str, user_id: str | None = None):
...
async def get_sandbox_by_id_safely(client, sandbox_id: str, start_if_stopped: bool = True) -> AsyncSandbox:
    ...
    # Find the project that owns this sandbox
    project_result = (
        await client.table("projects").select("project_id").filter("sandbox->>id", "eq", sandbox_id).execute()
    )
    ...
    try:
        ...
        logger.error(f"Error retrieving sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve sandbox: {e!s}") from e
...
async def create_file(
    ...
    try:
        ...
        logger.error(f"Error creating file in sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/sandboxes/{sandbox_id}/files")
async def list_files(
    sandbox_id: str, path: str, _request: Request = None, user_id: str | None = Depends(get_optional_user_id)
):
    ...
    try:
        ...
        logger.error(f"Error listing files in sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def should_include_file(file_path: str, excluded_dirs: set) -> bool:
    ...
    return bool(filename.startswith(".") and ext in {"js", "ts", "json", "yml", "yaml", "toml"})
...
async def get_file_tree(
    ...
    try:
        ...
        if not path:
            project_result = (
                await client.table("projects").select("app_type").filter("sandbox->>id", "eq", sandbox_id).execute()
            )
        ...
        filtered_files = [f for f in all_files if should_include_file(f, DEFAULT_EXCLUDED_DIRS)]
        ...
        return {"tree": tree, "totalFiles": len(filtered_files), "basePath": path}

    except Exception as e:
        logger.error(f"Error getting file tree for sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def _recursive_list_files_optimized(sandbox, base_path: str, max_depth: int = 10) -> list:
    ...
    return await fetch_directory(base_path, 0)
...
async def read_file(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error downloading file {path} from sandbox {sandbox_id}: {download_err!s}")
            raise HTTPException(status_code=404, detail=f"Failed to download file: {download_err!s}") from download_err
        ...
        return Response(
            content=content, media_type="application/octet-stream", headers={"Content-Disposition": content_disposition}
        )
    except HTTPException:
        # Re-raise HTTP exceptions without wrapping
        raise
    except Exception as e:
        logger.error(f"Error reading file in sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/sandboxes/{sandbox_id}/download-archive")
async def download_archive(
    sandbox_id: str, _request: Request = None, user_id: str | None = Depends(get_optional_user_id)
):
    ...
    try:
        ...
        project_result = (
            await client.table("projects").select("app_type, name").filter("sandbox->>id", "eq", sandbox_id).execute()
        )
        ...
        logger.info("Creating tar archive in sandbox")
        ...
        with (
            tarfile.open(fileobj=io.BytesIO(tar_content), mode="r:gz") as tar,
        ...
        logger.error(f"Error downloading archive for sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def delete_file(
    ...
    try:
        ...
        logger.error(f"Error deleting file in sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def ensure_project_sandbox_active(
    project_id: str, request: Request = None, user_id: str | None = Depends(get_optional_user_id)
):
    ...
    try:
        ...
        return {"status": "success", "sandbox_id": sandbox_id, "message": "Sandbox is active"}
    except Exception as e:
        logger.error(f"Error ensuring sandbox is active for project {project_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def execute_command(
    sandbox_id: str, request: Request = None, user_id: str | None = Depends(get_optional_user_id)
):
    ...
    try:
        ...
        if blocking:
            ...
            response = await sandbox.process.exec(command=command, cwd=cwd, timeout=timeout)
        ...
        req = SessionExecuteRequest(command=command, var_async=True, cwd=cwd)

        response = await sandbox.process.execute_session_command(session_id=session_name, req=req)
        ...
        logger.error(f"Error executing command in sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e


@router.get("/sandboxes/{sandbox_id}/sessions/{session_name}/status")
async def get_session_status(
    sandbox_id: str, session_name: str, _request: Request = None, user_id: str | None = Depends(get_optional_user_id)
):
    ...
    try:
        ...
        for command in session.commands:
            try:
                logs = await sandbox.process.get_session_command_logs(session_id=session_name, command_id=command.id)
                commands_info.append({"command": command.command, "exit_code": command.exit_code, "logs": logs})
            except Exception as e:
                commands_info.append(
                    {"command": command.command, "exit_code": command.exit_code, "error": f"Failed to get logs: {e!s}"}
                )

        return {"session_name": session_name, "commands": commands_info, "total_commands": len(session.commands)}

    except Exception as e:
        logger.error(f"Error getting session status for sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def get_sandbox_preview_url(
    ...
    try:
        ...
        try:
            ...
            return {"preview_url": proxy_url, "status": "available"}

        except Exception as preview_error:
            logger.error(f"Could not get preview link for sandbox {sandbox_id} on port {port}: {preview_error!s}")

            # Try to check if dev server is running on the correct port
            try:
                health_check = await sandbox.process.exec(
                    f"curl -s http://localhost:{port} -o /dev/null -w '%{{http_code}}' || echo '000'", timeout=10
                )
                ...
                if http_code == "000" or not http_code:
                    return {"preview_url": None, "status": "dev_server_not_running"}
                return {"preview_url": None, "status": "preview_not_available"}
            except Exception as health_error:
                logger.warning(f"Health check failed for sandbox {sandbox_id}: {health_error}")
                return {"preview_url": None, "status": "error"}

    except Exception as e:
        logger.error(f"Error getting preview URL for sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=f"Error getting preview URL: {e!s}") from e
...
async def get_expo_url(sandbox_id: str, user_id: str = Depends(get_optional_user_id)):
    ...
    is more reliable than parsing terminal logs. Includes a single retry with 2s delay
    if the initial Daytona call fails (handles transient failures during sandbox boot).
    ...
    try:
        ...
        if not sandbox:
            return {"expo_url": None, "status": "sandbox_not_running"}
        ...
        # RETRY: If first get_preview_link() failed, wait briefly and try once more
        # (handles transient Daytona failures during sandbox boot)
        logger.info(f"[EXPO URL] Retrying get_preview_link after 2s for sandbox {sandbox_id}")
        ...
        try:
            ...
            if daytona_url:
                if daytona_url.startswith("https://"):
                    ...
                    expo_url = f"exp://{daytona_url}"
                expo_url = expo_url.rstrip("/")
                logger.info(f"[EXPO URL] Retry succeeded for sandbox {sandbox_id}: {expo_url}")
                return {
                    "expo_url": expo_url,
                    "daytona_url": daytona_url,
                    "status": "available",
                    "method": "daytona_preview_link_retry",
                }
        except Exception as retry_error:
            logger.warning(f"[EXPO URL] Retry also failed: {retry_error}")
        ...
        logger.error(f"Error getting Expo URL for sandbox {sandbox_id}: {e!s}")
        raise HTTPException(status_code=500, detail=f"Error getting Expo URL: {e!s}") from e
...
async def stream_dev_server_status(
    ...
    async def event_generator():
        ...
        try:
            ...
            if app_type == "mobile":
                ready_patterns = ["Metro waiting on", "Your app is ready", "8081"]
                port = 8081
            else:
                    ...
                    while check_count < max_checks:
                        ...
                        try:
                            logs_response = await sandbox.process.get_session_command_logs(
                                session_id=session_name, command_id=command_id
                            )
