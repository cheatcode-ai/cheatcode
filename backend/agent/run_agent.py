...
async def run_agent_run_stream(
    ...
    async def listen_for_stop_signal():
        ...
        try:
            ...
            logger.info(f"Stop signal listener cancelled for {agent_run_id} (Instance: {instance_id})")
        except Exception as e:
            logger.error(f"Error in stop signal listener for {agent_run_id}: {e}", exc_info=True)
    ...
    try:
        ...
        if final_status == "running":
            ...
            trace.start_span(name="agent_run_completed").end(status_message="agent_run_completed")
        ...
        try:
            ...
            logger.warning(f"Error flushing Langfuse data in agent run stream: {flush_error!s}")
...
async def update_agent_run_status(
    ...
    status: str,
    error: str | None = None,
    responses: list[dict[Any, Any]] | None = None,
) -> bool:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Invalid UUID format for agent_run_id: {agent_run_id}, error: {e!s}")
            return False
            ...
            try:
                ...
                if hasattr(update_result, "data") and update_result.data:
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to verify update for {agent_run_id}: {verify_error!s}")
                ...
                logger.error(f"Database error on retry {retry} updating status for {agent_run_id}: {error_str}")
        ...
        error_details = traceback.format_exc()
        logger.error(
            f"Unexpected error updating agent run status for {agent_run_id}: {e!s}. Error details: {error_details}"
        )
        return False
