...
class SandboxVisionTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_id: str, thread_manager: ThreadManager, app_type: str = "web"):
    ...
    def get_tool_schemas(self) -> dict[str, list[ToolSchema]]:
        ...
        if self.app_type == "mobile":
            # Expo React Native examples - mobile UI screenshots, app icons, etc.
            local_example = "assets/images/app-icon.png"
            screenshot_example = "screenshots/mobile-screen.png"
            description_context = "mobile app UI elements, screenshots, or design assets"
            xml_local_example = "assets/splash-screen.png"
            xml_url_example = "https://example.com/mobile-mockup.jpg"
        else:
            # React/Next.js examples - web screenshots, UI mockups, etc.
            local_example = "public/images/hero-banner.jpg"
            screenshot_example = "screenshots/web-page.png"
            description_context = "web page screenshots, UI mockups, or design assets"
            xml_local_example = "public/favicon.ico"
            xml_url_example = "https://example.com/web-design.png"
        ...
        schemas["see_image"] = [
            ToolSchema(
                ...
                schema={
                    ...
                    "function": {
                        "name": "see_image",
                        "description": f"Allows the agent to 'see' an image file located in the workspace directory or from a URL. Perfect for analyzing {description_context}. Provide either a relative path to a local image or the URL to an image. The image will be compressed before sending to reduce token usage. The image content will be made available in the next turn's context.",
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "file_path": {
                                    "type": "string",
                                    "description": f"Either a relative path to the image file within the workspace directory (e.g., '{local_example}', '{screenshot_example}') or a URL to an image (e.g., 'https://example.com/image.jpg'). Supported formats: JPG, PNG, GIF, WEBP. Max size: 10MB.",
                                }
                            },
                            "required": ["file_path"],
                        },
            ...
            ToolSchema(
                ...
                xml_schema=XMLTagSchema(
                    ...
                    example=f"""
<!-- Example: Request to see a local image -->
<function_calls>
<invoke name="see_image">
<parameter name="file_path">{xml_local_example}</parameter>
</invoke>
</function_calls>
...
<function_calls>
<invoke name="see_image">
<parameter name="file_path">{xml_url_example}</parameter>
</invoke>
</function_calls>
""",
                ),
    ...
    def download_image_from_url(self, url: str) -> tuple[bytes, str]:
        ...
        try:
            ...
            return self.fail_response(f"Failed to download image from URL: {e!s}")

    async def see_image(self, file_path: str) -> ToolResult:
        """Read an image file from local file system or from a URL, compress it, convert it to base64, and add it as a temporary message."""
        try:
            ...
            if is_url:
                ...
                if not mime_type or not mime_type.startswith("image/"):
                    ...
                    ext = PurePath(cleaned_path).suffix.lower()
                    if ext in {".jpg", ".jpeg"}:
            ...
            return self.fail_response(f"An unexpected error occurred while trying to see the image: {e!s}")
