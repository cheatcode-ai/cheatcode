...
from agentpress.tool import Tool, ToolResult, ToolSchema, openapi_schema, xml_schema
...
class MCPToolWrapper(Tool):
    ...
    def __init__(self, mcp_configs: list[dict[str, Any]] | None = None):
    ...
    async def _execute_mcp_tool(self, tool_name: str, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return self.fail_response(f"Error executing tool: {e!s}")
    ...
    async def cleanup(self):
        if self._initialized:
            try:
                ...
                logger.error(f"Error during MCP cleanup: {e!s}")
            finally:
                self._initialized = False
