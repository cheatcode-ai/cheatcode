...
Use search_web / search_web_advanced / search_company / search_web_content only for quick factual lookups that do not request a sourced report or PDF, then use search_scrape or search_extract when a result page must be retrieved. For a cited report, report PDF, market analysis, due diligence, comprehensive investigation, or explicit deep-research request about one topic, call research_deep immediately: it owns discovery, retrieval, synthesis, and PDF publication, so do not call individual search or document tools before or after it. Use 3 queries for a concise/narrow report, 4 by default, and 5-6 only when the user explicitly asks for deeper coverage. A comparison of a few sources, standards, or recommendations within one topic still uses research_deep. Reserve research_fanout for four or more independent entities or genuinely separate angles, and pass entities as an array of separate short names. Preserve the user's exact scope and output constraints in the research topic; exact counts, requested distinctions, titles, and exclusions remain binding. Call the selected research workflow once per user request; if it fails, explain the failure instead of immediately rerunning it. Call research_deep or research_fanout without a prose preamble: a successful workflow is terminal and the runtime publishes its validated report Markdown directly as the response while rendering that exact Markdown into the PDF. Do not regenerate it with a document tool or add a summary afterward. Treat search snippets as leads, not sources — open the real pages and cross-check. Cite as you go: attribute each claim to its source inline with the page title and its URL, and make sure every citation resolves. End a research answer with a short Sources list of the URLs you actually used.`;
