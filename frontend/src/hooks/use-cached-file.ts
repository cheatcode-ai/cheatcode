 ...
 * @param appType App type to determine workspace directory
...
function normalizePath(path: string, appType: 'web' | 'mobile' = 'web'): string {
  ...
  // If the path already starts with the correct workspace, use it as-is
  if (path.startsWith(workspacePath)) {
    return handleUnicodeEscapes(path);
  }
  
  // If it's a relative path (doesn't start with /workspace), prepend the workspace
  if (!path.startsWith('/workspace')) {
    const cleanPath = path.startsWith('/') ? path.substring(1) : path;
    return handleUnicodeEscapes(`${workspacePath}/${cleanPath}`);
  }
  
  // If the path starts with /workspace but not the correct workspace,
  // this likely indicates a mismatch between the path and appType.
  // Return the path as-is to avoid corruption - let the caller handle it.
  return handleUnicodeEscapes(path);
}

/**
 * Handle Unicode escape sequences in a path
 * @param path The path to process
 * @returns Path with Unicode escapes resolved
 */
function handleUnicodeEscapes(path: string): string {
  try {
 ...
 */
function getCacheKey(sandboxId: string, path: string, appType: 'web' | 'mobile' = 'web'): string {
...
export function useCachedFile<T = string>(
  ...
  const cacheKey = sandboxId && filePath
    ? getCacheKey(sandboxId, filePath, options.appType || 'web')
    : null;
  ...
  const getCachedFile = async (key: string, force = false) => {
    ...
    try {
      ...
      const normalizedPath = normalizePath(filePath || '', options.appType || 'web');
      ...
      const isImageFilePath = filePath ? FileCache.isImageFile(filePath) : false;
      const isPdfFilePath = filePath ? FileCache.isPdfFile(filePath) : false;
      const isBinaryFile = isOfficeFile || isImageFilePath || isPdfFilePath;
  ...
  }, [sandboxId, filePath, options.contentType, options.appType]);
...
export const FileCache = {
  ...
  preload: async (sandboxId: string, filePaths: string[], token?: string | null, appType: 'web' | 'mobile' = 'web') => {
    if (!token) {
    ...
    const preloadPromises = uniqueFilePaths.map(async (path) => {
      ...
      const normalizedPath = normalizePath(path, appType);
      const key = getCacheKey(sandboxId, normalizedPath, appType);
...
export async function getCachedFile(
  ...
  const normalizedPath = normalizePath(filePath, options.appType || 'web');
  const key = getCacheKey(sandboxId, normalizedPath, options.appType || 'web');
...
export async function fetchFileContent(
  ...
  const attemptFetch = async (isRetry: boolean = false): Promise<string | Blob | any> => {
    try {
      // Prepare the API URL
      const apiUrl = `${API_URL}/sandboxes/${sandboxId}/files/content`;
