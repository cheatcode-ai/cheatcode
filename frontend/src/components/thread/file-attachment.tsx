...
function getFileUrl(
  ...
  // Check if the path already starts with the correct workspace path
  if (path.startsWith(workspacePath)) {
    ...
    const cleanRelativePart = relativePart.replace(
      /^(cheatcode-app|cheatcode-mobile)\//,
      '',
    );
    path = cleanRelativePart
      ? `${workspacePath}/${cleanRelativePart}`
      : workspacePath;
  } else {
    // Relative path, make it relative to the correct workspace
...
export function FileAttachment({
  ...
  const fileUrl =
    localPreviewUrl ||
    (sandboxId ? getFileUrl(sandboxId, filepath, project?.app_type) : filepath);
  const typeLabel = getTypeLabel(fileType, extension);
  const fileSize = getFileSize(filepath, fileType);
  const IconComponent = useMemo(() => getFileIcon(fileType), [fileType]);
  ...
  if (isImage && showPreview) {
    ...
    const imageHeight = isGridLayout
      ? ((customStyle as Record<string, string>)?.['--attachment-height'] ??
        '180px')
      : '54px';
    ...
    return (
      ...
      >
        <Image
          ...
          sizes={isGridLayout ? '100vw' : '200px'}
          onLoad={() => {
