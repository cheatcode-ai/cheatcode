...
export function SiteHeader() {
  ...
  const { toggleSidePanel, isMobile, debugMode, handleProjectRenamed } =
    useLayout();
  ...
  const [isUpdatingDeployment, setIsUpdatingDeployment] =
  ...
  const { data: deploymentStatus, isLoading: isLoadingDeploymentStatus } =
    useQuery<{
      ...
      queryFn: async () => {
        ...
        const response = await apiClient.get(
          `/project/${projectId}/deployment/status`,
        );
  ...
  const { data: liveStatus } = useQuery<{
    ...
    queryFn: async () => {
      ...
      const apiClient = createClerkBackendApi(getToken);
      const response = await apiClient.get(
        `/project/${projectId}/deployment/live-status`,
      );
      return response.success
        ? (response.data as { state?: string; message?: string })
        : null;
    },
        ...
        <div className="flex items-center gap-6">
          ...
          {!isMobile && (
            <div className="flex items-center gap-4">
              ...
              {projectId &&
              project?.app_type ===
                'mobile' ? null : isLoadingDeploymentStatus ? (
                <Skeleton
                  className={cn('h-8 w-24 rounded-md', threadStyles.skeleton)}
                />
              ) : deploymentStatus?.has_deployment ? (
                <Popover
                  open={deployPopoverOpen}
                  onOpenChange={setDeployPopoverOpen}
                >
                  ...
                  <PopoverContent
                    className={cn(
                      'w-80 rounded-lg shadow-2xl p-0 overflow-hidden font-mono',
                      threadStyles.card,
                    )}
                  >
                    <div className="p-5 space-y-4">
                      <div>
                        <div className="flex items-center justify-between mb-1.5">
                          <h4 className="text-xs font-medium text-thread-text-primary uppercase tracking-wide">
                            Deployment
                          </h4>
                          <span
                            className={cn(
                          ...
                          <div className="flex items-center justify-between text-[10px] font-mono text-thread-text-secondary uppercase tracking-wider">
                            <span>
                              {liveStatus?.state === 'preparing'
                                ? 'Preparing...'
                                : liveStatus?.state === 'pushing'
                                  ? 'Pushing...'
                                  : liveStatus?.state === 'building'
                                    ? 'Building...'
                                    : liveStatus?.state === 'deploying'
                                      ? 'Deploying...'
                                      : isUpdatingDeployment
                                        ? 'Redeploying...'
                                        : 'Preparing...'}
                      ...
                      {/* Deployed site link */}
                      {deploymentStatus?.domains &&
                        deploymentStatus.domains.length > 0 && (
                          <div className="space-y-2">
                            {deploymentStatus.domains.map(
                              (domain: string, index: number) => (
                                ...
                                >
                                  ...
                                  <span className="text-xs truncate font-mono underline decoration-thread-border underline-offset-4 group-hover:decoration-thread-text-tertiary">
                                    {domain}
                                  </span>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            await apiClient.post(
                              `/project/${projectId}/deploy/git/update`,
                              {},
                              {
                                timeout: 600000,
                              },
                ...
                >
                  <PopoverTrigger asChild>
                    ...
                    >
                      <Rocket
                        className={`w-3.5 h-3.5 ${isDeploying ? 'animate-pulse' : ''}`}
                      />
                    ...
                    <div className="p-5 space-y-5">
                      <div>
                        ...
                        <p className="text-[10px] text-thread-text-tertiary mt-1.5 leading-normal">
                          Deploy your project to get a live URL accessible from
                          anywhere.
                        </p>
                      </div>
                          ...
                          try {
                            ...
                            if (res.success) {
                              const data = res.data as
                                | { domains?: string[] }
                                | undefined;
                              const list = (data?.domains || []).filter(
                                Boolean,
                              );
                              toast.success(
                                `Deployed${list.length ? ` @ ${list.join(', ')}` : ''}`,
                              );
                              setDeployPopoverOpen(false);
