...
export function AttachmentGroup({
  ...
  const uniqueFiles =
    typeof files[0] === 'string' ? ([...new Set(files)] as string[]) : files;
  ...
  const visibleFilesWithMeta = uniqueFiles
    .slice(0, visibleCount)
    .map((file, _index) => {
      const path = getFilePath(file);
      const filename = path.split('/').pop() || '';
      const isImage =
        filename.match(/\.(jpg|jpeg|png|gif|webp|svg|bmp)$/i) !== null;
      return {
        file,
        path,
        isImage,
  ...
  const sortedFiles = [...uniqueFiles].sort((a, b) => {
    ...
    const isPreviewFile = (file: string | UploadedFile) =>
    ...
    return aIsUncollapsedPreview === bIsUncollapsedPreview
      ? 0
      : aIsUncollapsedPreview
        ? 1
        : -1;
  });
  ...
  const sortedFilesWithMeta = sortedFiles.map((file, index) => {
    ...
    return {
      ...
      wrapperStyle:
        shouldSpanFull || (isPreviewFile && !collapsed)
          ? { gridColumn: '1 / -1' }
          : undefined,
    };
  });
                ...
                customStyle={
                  item.isImage
                    ? {
                        ...
                        ...({
                          '--attachment-height': `${item.shouldSpanFull ? Math.floor(gridImageHeight * 1.33) : gridImageHeight}px`,
      ...
      return (
        <div className={cn('flex flex-wrap gap-3', className)}>
          {visibleFilesWithMeta.map((item, index) => (
            <div
              key={item.path}
              className={cn('relative group h-[54px]', item.wrapperClassName)}
            >
              <FileAttachment
                filepath={item.path}
                onClick={handleFileClick}
          ...
          >
            {(() => {
              ...
              return modalFilesWithMeta.map((item) => (
                <div key={item.originalIndex} className={item.wrapperClassName}>
                  <FileAttachment
                    filepath={item.path}
                    onClick={(path) => {
