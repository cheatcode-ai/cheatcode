...
import { type Thread, type Project } from '@/lib/api';
...
export function NavProjects() {
  ...
  const [threadToDelete, setThreadToDelete] = useState<{
    id: string;
    name: string;
    type: 'project' | 'conversation' | 'multiple';
  } | null>(null);
  ...
  const processAllProjects = (
    threads: Thread[],
    projects: Project[],
  ): ThreadWithProject[] => {
    const threadsWithProjects = processThreadsWithProjects(threads, projects);
    const projectsWithThreads = new Set(
      threadsWithProjects.map((t) => t.projectId),
    );
  ...
  const handleThreadClick = (
    ...
    e.preventDefault();
    setLoadingThread({ threadId, targetPath: url });
  ...
  const confirmDelete = async () => {
    ...
    if (threadToDelete.id.startsWith('project-')) {
      ...
      const threadIdsToDelete = Array.from(selectedThreads).filter(
        (id) => !id.startsWith('no-thread-'),
      );
      const projectsWithoutThreadsSelected = Array.from(selectedThreads).filter(
        (id) => id.startsWith('no-thread-'),
      ).length;
      ...
      if (threadIdsToDelete.length === 0) {
        toast.error(
          projectsWithoutThreadsSelected > 0
      ...
      if (projectsWithoutThreadsSelected > 0) {
        toast.warning(
          `${projectsWithoutThreadsSelected} project(s) without conversations will be skipped. Use "Delete Project" to delete them.`,
        );
      }
  ...
  return (
    <SidebarGroup className="p-0">
                      ...
                      >
                        <div className="flex items-center w-full gap-3">
                          ...
                          >
                              ...
                              {isThreadLoading ? (
                                ...
                                <>
                                  {thread.appType === 'mobile' ? (
                                    <Smartphone
                                      className={`h-4 w-4 transition-colors ${isActive ? 'text-white' : 'text-zinc-600 group-hover/row:text-zinc-400'}`}
                                    />
                                  ) : (
                                    <Monitor
                                      className={`h-4 w-4 transition-colors ${isActive ? 'text-white' : 'text-zinc-600 group-hover/row:text-zinc-400'}`}
                            ...
                            <span
                              className={`truncate text-sm font-medium ${isActive ? 'text-white' : 'text-zinc-500 group-hover/row:text-zinc-300'}`}
                            >
                              {thread.projectName}
                            </span>
                          </Link>
                          ...
                          <div className="flex items-center gap-2 opacity-0 group-hover/row:opacity-100 transition-opacity duration-200">
                            ...
                            <DropdownMenu>
                              ...
                              >
                                <DropdownMenuItem
                                  asChild
                                  className="rounded-none focus:bg-zinc-900 focus:text-white cursor-pointer py-2.5 px-3 border-b border-zinc-900 last:border-0"
                                >
                                  <a
                                    href={thread.url}
                                    target="_blank"
      ...
      {(_isDeletingSingle || isDeletingMultiple || isDeletingProject) &&
        totalToDelete > 0 && (
