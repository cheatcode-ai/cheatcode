...
function CompositoDashboardManagerComponent() {
  ...
  if (isLoading)
    return (
      <div className="p-12">
        <Skeleton className="h-12 w-full max-w-lg mx-auto bg-zinc-900" />
      </div>
    );
              ...
              >
                ...
                {tab === 'Connected' && profiles.length > 0 && (
                  <span
                    className={cn(
                      'ml-2 text-[10px]',
                      activeTab === tab ? 'text-black/60' : 'text-zinc-600',
                    )}
                  >
                    {profiles.length}
                  </span>
          ...
          {activeTab === 'Connected' && (
            ...
            >
              {profiles.map((p) => (
                <div
                  key={p.profile_id}
                  className="group relative bg-zinc-900 border border-zinc-800/50 rounded-3xl p-6 hover:bg-zinc-800 transition-all"
                >
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <Switch
                      ...
                      onCheckedChange={() =>
                        handleToggle(p.profile_id, p.is_default_for_dashboard)
                      }
                  ...
                  <div className="text-xs text-zinc-500 mb-6 flex items-center gap-2">
                    <span
                      className={cn(
                        ...
                        p.is_connected ? 'bg-emerald-500' : 'bg-red-500',
                      )}
                    />
...
export const CompositoDashboardManager = memo(
  CompositoDashboardManagerComponent,
);
