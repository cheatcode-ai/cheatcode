  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-zinc-600 line-clamp-1">
        {toolkit.categories[0] || 'Integration'}
      </div>
...
export const ComposioRegistry: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useComposioToolkits({ search: search || undefined });
  ...
  return (
    <div className="space-y-10">
      ...
      {isLoading ? (
        <div className="flex justify-center py-20 text-zinc-600">
          <Loader2 className="animate-spin" />
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {toolkitsData?.toolkits?.map((t: ComposioToolkit) => (
            <ToolkitCard
              key={t.slug}
              toolkit={t}
