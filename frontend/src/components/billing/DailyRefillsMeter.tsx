...
export function DailyRefillsMeter({ className }: DailyRefillsMeterProps) {
  ...
  const refillsUsed = Math.min(
    Math.ceil(creditsUsed / creditsPerRefill),
    maxRefills,
  );
  ...
  const hoursUntilRefill = Math.ceil(
    ...
    <div className="max-w-sm space-y-3 text-foreground">
      ...
      <ul className="text-sm space-y-1 text-foreground/90">
        <li className="flex items-start gap-2">
          ...
          <span>5 credits added each day (up to 4×/month)</span>
        </li>
        <li className="flex items-start gap-2">
          <CheckCircle className="h-4 w-4 text-emerald-400 mt-0.5" />{' '}
          <span>Refills reset at midnight</span>
        </li>
        <li className="flex items-start gap-2">
          <CheckCircle className="h-4 w-4 text-emerald-400 mt-0.5" />{' '}
          <span>Unused refills don’t carry over</span>
        </li>
        <li className="flex items-start gap-2">
          <CheckCircle className="h-4 w-4 text-emerald-400 mt-0.5" />{' '}
          <span>Maximum 20 credits per month</span>
        </li>
      </ul>
      {refillsRemaining > 0 ? (
        <div className="text-sm bg-blue-600/15 border border-blue-400/30 text-blue-100 p-3 rounded-md">
          <div className="font-medium">
            Next refill in ~{hoursUntilRefill} hours
          </div>
        ...
        <div className="text-sm bg-amber-600/15 border border-amber-400/30 text-amber-100 p-3 rounded-md">
          ...
          <div>
            You've used all 4 daily refills this month. Upgrade for more
            credits.
          </div>
        </div>
      )}
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="flex items-center gap-2 text-muted-foreground">
            ...
            <Tooltip delayDuration={0}>
              ...
              <TooltipContent
                side="top"
                className="p-4 rounded-md border ring-1 ring-border bg-background text-foreground shadow-xl"
              >
                {tooltipContent}
              </TooltipContent>
            </Tooltip>
