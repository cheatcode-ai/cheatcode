...
// Segmented Progress Bar (Orange) - extracted to module level
function CreditSegmentedProgress({
  value,
  total = 50,
}: {
  value: number;
  ...
  return (
    <Card className="w-full bg-[#111] border-zinc-800 shadow-xl overflow-hidden rounded-2xl">
      <CardContent className="p-8 space-y-6">
        ...
        {/* Progress Bar */}
        <div>
          <CreditSegmentedProgress value={creditsUsagePercentage} />
          <div className="flex justify-between text-[10px] font-mono text-zinc-500 uppercase tracking-wider">
            <span>
              {Math.round(creditsTotal - creditsRemaining).toLocaleString()} /{' '}
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <Link
              ...
              className="h-6 flex items-center justify-center text-xs rounded-full px-3 border border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors"
            >
              View all
            </Link>
          </div>
          ...
          <Table>
            <TableHeader className="bg-transparent hover:bg-transparent">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="text-[10px] font-mono uppercase text-zinc-600 h-8 pl-0">
                  Date
                </TableHead>
                <TableHead className="text-[10px] font-mono uppercase text-zinc-600 h-8">
                  Model
                </TableHead>
                <TableHead className="text-[10px] font-mono uppercase text-zinc-600 h-8 text-right">
                  Credits
                </TableHead>
                <TableHead className="text-[10px] font-mono uppercase text-zinc-600 h-8 text-right pr-0">
                  Cost
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {history.map((item, i) => (
                <TableRow
                  key={i}
                  className="hover:bg-zinc-900/50 border-zinc-800/30 text-xs font-mono group transition-colors"
                >
                  <TableCell className="py-2.5 pl-0 text-zinc-500 group-hover:text-zinc-400">
                    {item.date}
                  </TableCell>
                  <TableCell className="py-2.5 text-zinc-400 group-hover:text-white">
                    {item.model}
                  </TableCell>
                  <TableCell className="py-2.5 text-right text-zinc-500 group-hover:text-zinc-400">
                    {item.credits}
                  </TableCell>
                  <TableCell className="py-2.5 text-right pr-0 text-zinc-400 group-hover:text-white">
                    {item.cost}
                  </TableCell>
                </TableRow>
              ))}
